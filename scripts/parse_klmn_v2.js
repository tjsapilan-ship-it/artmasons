...
function parseRawText(text, letter) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Pattern 2: Short range like "1868-69"
    else if (line.match(/^\d{4}-\d{2}$/)) {
      lifePatterns.push({ index: i, life: line, type: 'single' });
    }
    // Pattern 3: Born pattern "b.1976"
    else if (line.match(/^b\.\d{4}$/)) {
      lifePatterns.push({ index: i, life: line, type: 'single' });
    }
    // Pattern 4: Split across lines "YYYY-" or "YYYY -" then "YYYY"
    else if (line.match(/^\d{4}$/) && i > 0) {
      const prevLine = lines[i-1].trim();
      // Match "1866-" or "18040-" or "1836 -" (with space)
      if (prevLine.match(/^\d{4,5}\s*-?\s*$/)) {
...
function main() {
  ...
  console.log('KLMN PARSER - IMPROVED');
  console.log('='.repeat(80));
