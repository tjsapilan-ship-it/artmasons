/**
 * Popular Portrait Artworks
 * 
 * This file contains a curated list of portrait artwork slugs.
 * All artworks reference existing records from the main artworks data.
 */

export const POPULAR_PORTRAITS = [
  // Leonardo Da Vinci
  'mona-lisa-la-gioconda',
  
  // Johannes Vermeer Van Delft
  'the-girl-with-the-pearl-earring',
  
  // Gustav Klimt
  'portrait-of-adele-bloch-bauer-i',
  'portrait-of-johanna-straude',
  'portrait-of-adele-bloch-bauer-ii',
  
  // Vincent Van Gogh
  'self-portrait-with-bandaged-ear-and-pipe',
  'portrait-of-doctor-gachet',
  'portrait-of-dr-felix-rey',
  'portrait-of-the-postman-joseph-roulin',
  
  // Grant Wood
  'american-gothic',
  
  // Pablo Picasso
  'dora-maar',
  'bust-of-a-woman-and-self-portrait',
  'portrait-of-a-woman-in-dhermine-pass',
  'self-portrait-picasso',
  
  // Rembrandt Van Rijn
  'self-portrait-rembrandt-von-rijn',
  'self-portrait-rembrandt',
  
  // Jacques-Louis David
  'napoleon-crossing-the-alps-on-20th-may-1800-1803',
  'emporer-napoleon-i',
  
  // Barthel Beham
  'portrait-ludwig-x-of-bavaria',
  
  // Giovanni Bellini
  'young-woman-laura',
  
  // Giovanni Boldini
  'portrait-of-james-abbott',
  
  // Philippe De Champaigne
  'portrait-of-cardinal-richelieu',
  'triple-portrait-of-cardinal-richelieu',
  'portrait-of-a-man-possibly-of-robert-arnauld-of-andilly',
  
  // Georges Clarin
  'portrait-of-sarah-bernhardt',
  
  // Jean The Younger Clouet
  'portrait-of-francis-i-roi-de-france',
  
  // Francois Clouet
  'portrait-of-charles-ix',
  'portrait-of-henri-ii-king-of-france',
  'portrait-of-odet-de-coligny-cardinal-of-chatillon',
  'portrait-of-mary-queen-of-scots',
  'portrait-henri-vi-england',
  'portrait-of-catherine-de-medici',
  'portrait-king-charles-ix-of-france',
  'portrait-of-elizabeth-of-austria-queen-of-france',
  'portrait-of-louise-de-lorraine',
  
  // Frank Weston Benson
  'portrait-of-my-daughters',
  
  // Hilaire Germaine Edgar Degas
  'portrait-of-emma-dobigny',
  
  // Anselm Feuerbach
  'nanna',
  'nanna-2',
  'self-portrait-feuerbach',
  
  // Francois Flameng
  'portrait-of-princess-zinaida-yusupova-with-two-sons-at-arkhangelskoe',
  'family-portrait-of-a-boy-and-his-two-sisters',
  
  // Hippolyte Flandrin
  'madam-hippolyte-flandrin',
  'marie-anne-de-bourbon-duchesse-de-vendome',
  
  // Thomas Gainsborough
  'the-woman-in-blue-portrait-of-duchess-of-beaufort',
  
  // Anne-Louis Girodet De Roussy-Trioson
  'napoleon-in-coronation-robes',
  'portrait-of-young-man-as-a-hunter',
  
  // Frans Hals
  'portrait-of-a-young-man-holding-a-glove',
  'laughing-cavallier',
  'portrait-of-a-man-hals',
  'portrait-of-a-woman',
  'portrait-of-a-elderly-man',
  'malle-babbe',
  
  // Barbara Kraftt
  'portrait-of-wolfang-amadeus-mozart',
  
  // Edward Charles Tarbell
  'portrait-of-a-lady',
  
  // James Jacques Joseph Tissot
  'the-portrait-miss-lloyd',
  
  // Jan Weenix
  'dead-hare-and-partridges',
];
